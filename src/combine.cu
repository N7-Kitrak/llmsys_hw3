...
__global__ void mapKernel(
  ...
  if (out_pos < out_size) {
    ...
    int out_write_pos = index_to_position(out_index, out_strides, shape_size);
    out[out_write_pos] = fn(fn_id, in_storage[in_pos]);
  }
}
...
__global__ void reduceKernel(
  ...
  if (out_pos < out_size) {
    to_index(out_pos, out_shape, out_index, shape_size);
    float reduced_val = reduce_value;
    for (int i = 0; i < a_shape[reduce_dim]; ++i) {
      ...
      reduced_val = fn(fn_id, reduced_val, a_storage[a_pos]);
    }
    int out_write_pos = index_to_position(out_index, out_strides, shape_size);
    out[out_write_pos] = reduced_val;
  }
}
...
__global__ void zipKernel(
  ...
  if (out_pos < out_size) {
    ...
    int out_write_pos = index_to_position(out_index, out_strides, out_shape_size);
    ...
    out[out_write_pos] = fn(fn_id, a_storage[a_pos], b_storage[b_pos]);
  }
}
